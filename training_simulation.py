    ...
    def _replay(self):
        ...
        if len(batch) > 0:  # if the memory is full enough
            ...
            if isinstance(self._Model, TrainModel_DDQN):
                ...
                td_errors = np.zeros(len(batch))  # Initialize TD errors
                ...
                for i in range(len(batch)):
                    ...
                    target_q = reward + self._gamma * target_q_value  # calculate target Q(state, action)
                    td_errors[i] = target_q - current_q[action]  # calculate TD error
                    current_q[action] = target_q  # update Q(state, action)
                ...
                for i, b in enumerate(batch):
                    ...
                    current_q = q_s_a[i]  # get the Q(state) predicted before
                    target_q = reward + self._gamma * np.amax(q_s_a_d[i])  # calculate target Q(state, action)
                    td_errors[i] = target_q - current_q[action]  # calculate TD error
                    current_q[action] = target_q  # update Q(state, action)
            ...
            self._Memory.update_priorities(idxs, td_errors)  # update memory priorities
